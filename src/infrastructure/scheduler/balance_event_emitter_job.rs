...
use log::info;
use rdkafka::{producer::{FutureProducer, FutureRecord}, ClientConfig, Message};
...
impl BalanceEventEmitterJob {
    pub async fn publish_event(&self) {
        let _ = std::panic::catch_unwind(std::panic::AssertUnwindSafe(async || {
            self.do_publish_event().await;
        }));
    }

    async fn do_publish_event(&self) {
        info!("Start publishing event");
